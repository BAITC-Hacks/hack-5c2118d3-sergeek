...
import type { Campaign, Channel, DashboardData, DataLimits, Pilot, SimulationData } from '../types/api'

const apiBase = import.meta.env.VITE_API_URL ?? ''
type Source<T> = Promise<{ data: T; source: 'api' | 'demo' }>

type ApiDashboard = {
  baseline_arpu: number; net_gain: number; total_cost: number; total_contacts: number; pilot_count: number; final_campaign_count: number; status: 'PASS' | 'FAIL'
  limits: { budget: number; contacts: number; pilots: number; campaigns: number; customers_per_campaign: number }
  channels: Record<string, { cost_per_contact: number; conversion_multiplier: number }>; audience_size: number
}

type ApiCampaign = { campaign_name: string; current_tariff: string | null; arpu_segment: string | null; target_tariff: string; channel: string; audience_size: number; communication_cost: number; gross_lift: number; net_gain: number; status: string }
type ApiPilot = { name: string; customers: number; observed_lift_ratio: number; channel: string; cost: number }
...
function titleChannel(channel: string): Channel {
  const names: Record<string, Channel> = { push: 'Push', sms: 'SMS', digital_ads: 'Digital ads', call: 'Call' }
...
export const api = {
  dashboard: async (): Source<DashboardData> => {
    try {
      const [overview, campaignResponse] = await Promise.all([fetchApi<ApiDashboard>('/api/dashboard'), fetchApi<{ items: ApiCampaign[] }>('/api/campaigns')])
      const mappedCampaigns = campaignResponse.items.map(mapCampaign)
      const channelBudget = Object.values(mappedCampaigns.reduce<Record<string, number>>((totals, item) => ({ ...totals, [item.channel]: (totals[item.channel] ?? 0) + item.communicationCost }), {})).length
        ? Object.entries(mappedCampaigns.reduce<Record<string, number>>((totals, item) => ({ ...totals, [item.channel]: (totals[item.channel] ?? 0) + item.communicationCost }), {})).map(([name, value]) => ({ name, value })) : mockDashboard.channelBudget
      return { source: 'api', data: { baselineArpu: overview.baseline_arpu, campaignNetGain: overview.net_gain, budgetUsed: overview.total_cost, budgetLimit: overview.limits.budget, contactsUsed: overview.total_contacts, contactsLimit: overview.limits.contacts, pilotsUsed: overview.pilot_count, pilotsLimit: overview.limits.pilots, finalCampaigns: overview.final_campaign_count, finalCampaignLimit: overview.limits.campaigns, status: overview.status, channelBudget, campaignNet: mappedCampaigns.map((item) => ({ name: item.name, value: Math.round(item.expectedNetGain / 1000) })), audienceCoverage: [{ name: 'Reached', value: overview.total_contacts }, { name: 'Remaining', value: Math.max(0, overview.limits.contacts - overview.total_contacts) }] } }
  ...
  runSimulation: async (seedStart: number): Source<SimulationData> => { try { return { data: mapSimulation(await fetchApi<ApiSimulation>('/api/simulations/run', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ runs: 10, seed_start: seedStart }) })), source: 'api' } } catch { return { data: mockSimulation, source: 'demo' } } },
}
